...
Passiamo ora a discutere il metodo HardFixing realizzato, fornendo una panoramica del suo funzionamento e definendo quali scelte sono state adottate. La soluzione di partenza c_P è stata ottenuto applicando il metodo NearestNeightbor precedentemente discusso. Come accennato, i matheuristici risultano tipicamente applicati a soluzioni  con costi già particolarmente buoni, per tale ragione si è invocato il metodo TwoOpt su c_P il quale produrrà una soluzione x il cui costo risulta,tipicamente, inferiore rispetto a c_P. Fra le politiche possibili per esplorare N(x), si è scelto nuovamente di utilizzare \textbf{first improvement}.//
...
Poter disporre di un incumbent è molto importante poichè consente a Cplex di dichiarare sondati nodi il cui costo della soluzione del rilassamento continuo risulta essere superiore rispetto al costo dell' incumbent(criterio di bounding). Avere un incumbent consente inoltre a Cplex di utilizzare euristici che necessitano di esso, come ad esempio gli euristici \textbf{RINS} e \textbf{polishing} che saranno discussi nel paragrafo X. 

----Come si fa ad inserire

La soluzione di partenza \textbf{x} è stata fornita a Cplex come MIP start, inoltre qualora l' esplorazione di un intorno non determina una soluzione migliorativa rispetto a quella corrente x_I,  quest' ultima viene passata come warm start anche per  l'iterazione successiva di Cplex: in questo modo l' incumebent contiene sempre la soluzione che si vuole migliorare e, per applicare correttamente la politica first improvement, è sufficiente quindi arrestare il solver non appena l' incumbent viene aggiornato.

Si osserva che non è invece necessario fornire a Cplex un warm start nel caso in cui l' incumbent x_I venga aggiornato poichè, nonostante Cplex fissi ad "infinito" il valore dell' incument ogni qual volta inizia a risolvere il subMIP corrente, esso verifica se quest' ultimo risulta essere una soluzione ammissibile per il nuovo subMIP e in tal caso lo installa come incumbent. Per costruzione x_I risulta essere una soluzione ammissibile anche del subMIP corrente poichè questo è ottenuto fissano in soluzione variabili che appartengono proprio a x_I.
...
La MIP informative callback viene invocata tutte le volte che un thread si accinge a risolvere un nodo dell' albero decisionale. 
In maniera del tutto analoga a quanto discusso per la lazy constraint callback, per poterla utilizzare  è necessario creare una classe attribuendoli un nome a piacere, la quale deve estendere la classe astratta \textbf{MIPInfoCallback} contenuta nella libreria di cplex ed infine implementare al suo interno il metodo Main che verrà invocato automaticamente dal sistema ogniqualvolta la callback viene invocata. Nel codice prodotto la classe è stata chiamata \textbf{TSPInformativeCallback}, per installare la callback è necessario eseguire il seguente codice:


cplex.Use(new TSPInformativeCallback ())


Come accennato nel paragrafo precedente, l' incumbent del B&C risulta pari alla soluzione che si desidera migliorare e quindi, ricordando che si è applicata una politica di esplorazione first improvement, quando l' incumbent viene aggiornato è necessario arrestare Cplex. 

Ciò non poteva essere fatto utilizzando la lazy constraint callback poichè interrompere cplex al suo interno significava non aggiornare l' incumbent. Per tale ragione è stato necessario installare la MIP informative callback: in questo modo non appena un thread prima di procedere nell' esecuzione del rilassamento continuo di un nodo constata che l'incumebent è stato aggiornato, eseguendo il metodo Abort() ereditato dalla classe MIPInfoCallback arresta Cplex. Per verificare se l' incumbent attuale è inferiore a quello precedente, è necessario memorizzarlo in una variabile
...
Il metodo che realizza ciò è stato chiamato PreProcessing ed ha la seguente firma:
...
\begin{itemize}
    \item \textbf{instance}: 
    \item \textbf{x}: 
    \item \textbf{edgeFixed}: 
\end{itemize}

L' algoritmo realizzato ricerca in un primo momento tutte le componenti connesse presenti dopo il fissaggio: per far ciò si è utilizzato un vettore di interi denominato compConn in cui memorizziamo all' indice i la componente connessa del nodo i, e una lista di liste di interi chiamata nodesComp la quale all' indice i contiene una lista contenente tutti i nodi appartenente alla componente connessa i. Riportiamo di seguito il codice realizzato per inizializzare le strutture dati:
...
Una volta noti tutti i nodi che appartengono ad una generica componente connessa k, è necessario individuare i vertici \textit{i} e \textit{j} appartenenti a k nei quali incidono su di essi solo lato poichè, per costruzione, la variabile associata al lato [i,j] è la variabile cercata da fissare a 0. Individuare il lato [i,j] non risulta banale per cui si è preferito,data una componente connessa k, fissare sistematicamente a 0 tutte le variabili associate a lati i cui nodi indicenti appartengono alla componente connessa k e che non sono stati precedentemente fissati ad 1. In questo modo senz' altro si fissa a 0 la variabile che evita la formazione del subtour, in più si pongono a 0 variabili che durante la fase di pre-processing di Cplex comunque sarebbero state fissate a 0 per i vincoli di grado inseriti. Per cui, considerando la componente connessa formata dai vartici {6,7,8,9} di Fig.X, non solo viene fissata a 0 la variabile x_{6,9} ma anche le variabli x_{6,7} e x_{8,9} .

\begin{center}
\includegraphics[scale=0.5]{PreProcCompCon}  
...
La tecnica euristica precedentemente esposta è stata implementata all' interno del metodo HardFixing appartenente alla classe TSP. La sua intestazione risulta essere:
