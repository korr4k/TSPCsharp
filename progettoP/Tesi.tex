...
In termini generali possiamo definire gli algoritmi \textbf{matheuristics} come algoritmi euristici di miglioramento che, partendo da una o più soluzioni ammissibili, sfruttano il \textbf{modello matematico} per calcolare soluzioni migliori.
In particolare sfruttare il modello matematico significa poter utilizzare un solver e, poichè essi sono estremamente efficienti anche nel risolvere istanze grandi a costo di semplificare il modello, è possibile realizzare algoritmi euristici che producono soluzioni particolarmente buone in brevissimo tempo.
Per quanto possa risultare semplice tale idea, algoritmi di questo tipo sono stati implementati solamente a partire dagli anni 2000. Il punto saliente i questi metodi consta nella determinazione del vicinato di una soluzione di partenza \textbf{x}. Tale scelta è tutt'altro che banale, ed è la chiave per ottenere un metodo efficiente. Uno schema comunemente usato è di scegliere un certo numero di variabili della soluzione di riferimento \textbf{x} e fissarle, lasciando libere altre. Questo modo di procedere è detto\textbf{ hard variable fixing}; il suo principale svantaggio è rappresentato nel difficile problema della scelta di quali variabili sia opportuno fissare e quali lasciare libere. In contrapposizione a questa tecnica è possibile pensare ad un differente modo di procedere, il cosiddetto \textbf{soft variable fixing}, che consiste invece nel far sì che un certo numero di variabili restino immutate, ma senza conoscere a priori quali siano, lasciando che sia il solver ad effettuare la scelta ottimale fra tutte quelle possibili.

\vspace{2\baselineskip}
HARD FIXING
\vspace{2\baselineskip} 

L' hard fixing è una tecnica euristica che, data una soluzione ammissibile di riferimento \textbf{x_H}, consiste nel fissare in soluzione variabili che fanno parte di \textbf{x_H}. La soluzione \textbf{x_H} può essere ottenuta utilizzando un qualunque algoritmo costruttivo oppure Cplex fornendogli un time limit che ragionevolmente possa essere considerato breve per l' istanza che si vuole risolvere. Esistono varie politiche di fissaggio dei lati. Una strategia possibile consiste nello scansionare tutti i lati che costituiscono x_H e fissare in soluzione le variabili ad essi associati con probabilità \textbf{p}. Maggiore è il numero di variabili fissate, minore saranno le operazioni di branching e quindi conseguentemente i nodi dell' albero decisionale.
Si osserva che qualora si fissi le variabili corrispondenti ai due lati che incidono in un nodo, Cplex nella fase di preprocessing fissa a 0 le variabili corrispondente ai rimanenti n-3 lati uscenti dal nodo grazie ai vincoli di grado inseriti nel modello. 
Fissare alcune variabili induce la definizione di un intorno N(x_H) al cui interno sono presenti tutti i tour aventi tali variabili fissate: l' esplorazione dell' intorno non avviene più tramite enumerazione come visto per gli algorimi migliorativi, ma avviene utilizzando Cplex fornendo a quest' ultimo il modello matematico modificato.
 
\vspace{2\baselineskip} 
WARM-START
\vspace{2\baselineskip} 

Il solver Cplex consente di fornirgli in input una soluzione euristica del problema: tale soluzione prende il nome di \textbf{warm-start} o \textbf{MIP start}. Risulta possibile, in generale, fornire più MIP start, in tal caso si parla di MIP starts multiplo. Quando si fornisce un MIP start, esso viene processato da Cplex durante la fase di ottimizzazione prima di iniziare il branch and cut. Qualora uno o più MIP start rappresenti una soluzione ammissibile, CPLEX installa la migliore di queste soluzioni come incumbent. Poter disporre di un incumbent fin dall'inizio del branch and cut è molto importante poichè consente a Cplex di dichiarare sondati nodi il cui costo della soluzione del rilassamento continuo risulta essere superiore rispetto al costo dell' incumbent(criterio di bounding). Avere un incumbent consente inoltre a Cplex di utilizzare euristici che necessitano di esso, come ad esempio gli euristici \textbf{RINS} e \textbf{polishing} che saranno discussi nel paragrafo X.
...
Per migliorare le performance dell' algoritmo risulta molto utile eseguire una fase di fissaggio di variabili ulteriore chiamato pre-processing caratteristica per il problema del Commesso Viaggiatore. Si consideri la figura X:
...
La variabile x_{z,j} dovrà necessariamente assumere il valore 0 essendo x_{z,j} e x_{z,h} al valore 1 altrimenti ci sarà un subtour. Poichè i vincoli di subtour elimination, a differenza dei vincoli di grado, non sono stati inseriti nel modello iniziale, durante il preprocessing non fissa a 0 la variabile x_{z,j}. Qualora il lato venisse scelto poco male, la funzione di separazione che si è implementata aggiungerà il corrispondente vincolo di subtour: tuttavia visto che è noto a priori che tale variabile deve essere fissata a 0, al fine di aumentare le prestazioni del metodo, si è realizzato un metodo chiamato PreProcessing appartenente alla classe Utility.
...
La soluzione ammissibile è stata ottenuta applicando il metodo NearestNeightbor precedentemente discusso e codificata all' interno del vettore \textbf{heuristicSol} di tipo \textbf{PathStandard}. Inizialmente si è deciso 
di fissare mediamente l' $80%$ dei lati: i miglioramenti saranno così molto frequenti ma tipicamente piccoli avendo fissato la stragrande maggioranza d elle variabili. 

Qualora Cplex, dopo k esecuzioni in cui ha migliorato la soluzione, non  produce nessuna soluzione migliore, una strategia potrebbe essere quella di passare subito a fissare una percentuale inferiore di lati; questo tipicamente non viene fatto e si preferisce riprovare \textbf{t} volte prima di diminuire la percentuale di fissaggio in modo da avere, almeno nella fase iniziale, più miglioramenti possibili. Il valore di \textbf{t} dipende ovviamente dall' applicazione e da quali sono lo esigenze: all' interno dell' algoritmo realizzato si è ritenuto ragionevole fissate t = 10. 
