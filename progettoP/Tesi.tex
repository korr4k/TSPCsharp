...
Il codice proposto, dopo minime modifiche dovute alla presenza della variabile \textbf{nEdges}\footnote{Maggiori dettagli \hyperref[sec:MetodoBuildModelS]{Metodo BuildModel}}, viene inserito all'interno di un apposito metodo \textbf{BuilModel} appartenente alla classe \textbf{Utility}, che oltre alla definizione del modello matematico restituisce il riferimento al vettore di variabili:
...
Il rilassamento continuo del modello matematico costruito nella precedente sezione \hyperref[sec:SezioneCinqueS]{Sezione Cinque}, essendo privo dei vincoli di \textit{subtour elimination}, non può fornire in genere risultati accettabili indipendentemente da quale tipologia di algoritmi venga applicata.

All'interno di questa sezione viene quindi introdotto il primo metodo di risoluzione \textbf{esatto}, denominato \textbf{LOOP}, che fornisce una tecnica molto semplice per aggiungere ad ogni sua iterazione una serie di vincoli al modello matematico necessari al raggiungimento della soluzione \textbf{ottima} globale.

Successivamente sono proposte varianti dell'algoritmo, nel complesso sempre \textbf{esatte}, che sfruttando una \textbf{iniziale} fase \textbf{euristica} mirano a ridurre i tempi di esecuzione.
...
L'idea alla base è molto semplice: inizialmente il modello matematico creato, anche se privo di alcun vincolo di \textit{subtour elimination}, viene risolto. La soluzione \textit{ottima} del rilassamento continuo viene processata individuando eventuali componenti connesse al suo interno. Una loro presenza in quantità superiore ad \textbf{uno} implica inevitabilmente un grafo \textbf{non connesso} e di conseguenza la soluzione attuale deve essere dichiarata \textbf{non valida}. A questo punto si costruiscono i vincoli che avrebbero impedito al rilassamento continuo di proporre tale soluzione e si procede con la loro aggiunta nel modello matematico.

Eseguito l'aggiornamento, il procedimento viene ripetuto dal principio partendo con una nuova risoluzione. Proprio questo ripetersi degli stessi identici passi sistematicamente, creando così un \textbf{loop}, fornisce il nome alla tecnica.

Naturalmente, trattandosi di una algoritmo, il ciclo \textit{loop} deve avere una o più condizioni di arresto. In questo caso specifico si tratta dell'individuazione di una sola componente connessa all'interno dell'ultima soluzione trovata: questa è di conseguenza \textbf{valida} ed anche l'\textbf{ottimo globale} ricercato.

Il fatto di partire con un modello matematico privo di vincoli di \textit{subtour elimination}, combinato ad una loro aggiunta mirata e molto graduale, garantisce alle prime iterazione del metodo \textbf{LOOP} tempi di esecuzione estremamente brevi rendendo le performance finali di questo algoritmo soddisfacenti\footnote{Si noti che ogni vincolo, una volta aggiunto al modello matematico, non deve essere rimosso durante le fasi successive dell'algoritmo. In questo modo i tempi di esecuzione delle varie iterazioni successive tendono a crescere.}.

L'implementazione del metodo \textbf{LOOP} richiede quindi lo sviluppo di una funzione in grado di:

\begin{itemize}
	\item Individuare la presenza di più subtour all'interno di una generica soluzione;
	\item Generare i vincoli di subtour elimination in congruenza al punto precedente;
\end{itemize}

In letteratura esistono molteplici modi per eseguire tali operazioni, quella proposta si rifà al famoso algoritmo di \text{Kruskal} per trovare un albero a costo minimo in un grafo connesso non orientato\footnote{Nello specifico la parte di interesse è quella che impedisce la formazione di molteplici \textit{componenti connesse}.}.

I dettagli realizzativi sono riportati all'interno dell'appendice nella sezione dedicata al metodo \hyperref[sec:UCCICCS]{UpdateCC} della classe \textbf{Utility}. Per il momento è sufficiente specificare solamente che al termine dell'utilizzo del suddetto metodo i seguenti oggetti:
...
vengono popolati in modo tale che \textbf{rcExpr} contenga le espressioni dei vincoli di \textit{subtour elimination} cercati mentre all'interno di \textbf{bufferCoeffRC} siano presenti il numero dei lati appartenenti ad ogni \textit{subtour} e quindi il termine noto delle precedenti espressioni maggiorato di una unità\footnote{Il codice è realizzato così che l'espressione di indice $i$ presente all'interno di \textbf{rcExpr} debba essere combinata con l'elemento $i$ in \textbf{bufferCoeffRC}.}.
...
Il metodo \hyperref[sec:MetodoLoopS]{LOOP} nella forma base presentata non garantisce sempre prestazioni accettabili, soprattutto per istanze \textit{TSP} complesse. Trascorso un certo numero di iterazione si corre il rischio di dover risolvere modelli matematici con un numero di vincoli così alto da rallentare eccessivamente l'intero processo. Nelle applicazioni reali questa situazione non è spesso accettabile in quanto sono richiesti tempi di esecuzione molto ridotti.

In alternativa, sono nati nel corso degli anni diversi gli algoritmi \textbf{euristici} i quali, grazie alla rinuncia di un risultato ottimo garantito, offrono tecniche risolutive molto veloci.

Nulla vieta che tali tecniche siano anche implementate in alcune specifiche fasi di algoritmi \textbf{esatti} così da poter sfruttare le migliori proprietà di entrambi. Nel \textit{LOOP}, ad esempio, si può pensare di introdurre una fase iniziale che produca velocemente vincoli di \textit{subtour elimination} \textbf{utili} per il l'algoritmo classico.
Esistono innumerevoli modi per implementare questa idea, segue una descrizione di quelli utilizzati:

\begin{itemize}
    \item \textbf{CPLEX}, durante la risoluzione del modello matematico attraverso la tecnica del \textbf{Branch\&Cut}, tiene sempre aggiornati due parametri fondamentali. Il primo, detto \textbf{lower bound}, è il miglior \textit{obbiettivo} che i restanti nodi da analizzare possono aspirare di raggiungere (non viene mai abbassato). Il secondo, detto valore \textbf{incumbent}, è il costo della miglior soluzione \textbf{intera} trovata (non viene mai alzato).
    
    Di default, al coincidere\footnote{In realtà il valore di default è pari ad una distanza relativa dello $0.01\%$.} di questi due valori \textbf{Cplex} termina il processo risolutivo in quanto l'ottimo globale è stato identificato. \'E altresì possibile specificare diversamente, imponendo un arresto prematuro al raggiungimento di una certa distanza relativa tra \textit{lower bound} e \textit{incumbent}.
    
    Il parametro in questione prende il nome di \textbf{EpGap} e può assumere qualsiasi valore compreso tra $0$->$0\%$ ed $1$->$100\%$. Durante la prima fase euristica del metodo \textit{LOOP} si può quindi pensare di variare tale parametro fino al raggiungimento della soluzione ottima in queste condizioni;
    \item Come indicato ad inizio paragrafo, i tempi di risoluzione del metodo \textit{LOOP}, come per tutti gli algoritmi, hanno una forte dipendenza dalla istanza analizzata. Anche se una analisi a priori non può fornire indicazioni precise, più il numero di lati che compongono il grafo connesso è alto più complessa tende a diventare la risoluzione dell'istanza.
    
    Limitarne il numero ha pertanto un impatto notevole in tal senso ed è possibile farlo settando a priori il valore assunto da alcune variabili del modello matematico. Tanto migliore risulta essere la \textit{previsione} così introdotta, migliori saranno i tempi di risoluzione richiesti.
    
    L'approccio proposto prevede che per ogni nodo si lascino \textit{attivi} solamente gli $m$ lati a costo minore che vi incidono. Non esiste un valore migliore in assoluto per la quantità $m$ ed in alcuni casi, se settato troppo basso, può rendere il modello matematico \textbf{privo} di alcuna soluzione;
    \item Una semplice utilizzo congiunto delle precedenti;
\end{itemize}

Si specifica che in genere una tecnica euristica necessita di uno specifico \textbf{tuning} in base all'istanza su cui viene applicata in modo tale da rendere al meglio delle proprie possibilità. Pertanto sia il valore di \textbf{EpGap} che di \textbf{m} vengono lasciati settabili dall'utente.

Ottenuta la soluzione ottima della fase euristica, è necessario utilizzare il classico algoritmo \textit{LOOP} \textbf{esatto} mantenendo però validi i vincoli di \textit{subtour elimination} trovati in precedenza: il valore di \textbf{EpGap} viene riportato a $1e^{-04}$ oppure tutti i lati del grafo sono riabilitati.\\

Nei successivi paragrafi, \hyperref[sec:EpGapS]{EpGap} e \hyperref[sec:MetodoBuildModelS]{Metodo BuildModel}, vengono esposti i dettagli algoritmici per la realizzazione delle due fasi euristiche proposte.

\subsection*{EPGAP}
\label{sec:EpGapS}

Indicando con \textbf{lb} e \textbf{bestInteger} rispettivamente il \textbf{lower bound} ed il costo \textbf{incumbent} discussi nel paragrafo precedente durante l'introduzione del parametro di \textit{Cplex} \textbf{EpGap}, la formula:

\begin{eqnarray}
&\displaystyle|bestNode - bestInteger|/(1e-10 + |bestInteger|)
\end{eqnarray}

determina il valore che deve scendere sotto la soglia di \textit{EpGap} per terminare la risoluzione del modello matematico.

Il settaggio di \textbf{EpGap}, che si ricorda poter assumere qualsiasi valore compreso tra $0$->$0\%$ ed $1$->$100\%$, risulta molto semplice: è sufficiente invocare il metodo \textbf{SetParam} sull'oggetto della classe \textbf{Cplex} su cui è definito il modello matematico da risolvere:

\begin{lstlisting}

//newValue è un valore double compreso tra 0 ed 1 (inclusi)
cplex.SetParam(CPLEX.DoubleParam.EpGap, newValue);

\end{lstlisting}

\subsection*{METODO BUILDMODEL}
\label{sec:MetodoBuildModelS}

Nel paragrafo \hyperref[sec:ModelloCSS]{Modello Matematico: Linguaggio C\#}, è stato presentato il metodo \textbf{BuildModel} della classe \textbf{Utility} il cui compito è di creare il modello matematico, privo dei vincoli di \textit{subtour elimination}, risolubile da CPLEX.

Veniva lasciato in sospeso il parametro \textit{nEdges}, questo corrisponde esattamente al valore $m$, descritto in \hyperref[sec:LoopEuristicoS]{Metodo LOOP Con Prima Fase Euristica}, e quindi per ogni nodo quanti dei lati meno costosi ed in esso incidenti siano da lasciare abilitati nel modello matematico.

A livello implementativo, per rendere inutilizzabili certi collegamenti è sufficiente non inserirli nel modello matematico oppure farlo ma fissandone sia il \textbf{lower} che l'\textbf{upper bound} a \textbf{0}. Quest'ultima opzione è quella più comoda da utilizzare in quanto, terminata la fase euristica, tutti i lati devono essere abilitati per procedere e quindi basta riportare gli \textbf{upper bound} al valore \textbf{1}.

Diversi algoritmi che verranno presentati nel corso della tesi necessitano di conoscere per ogni nodo l'ordinamento \textbf{completo} dei lati in esso incidenti basato sul loro costo. La funzione \textbf{OrderEdgesComplete} presentata all'interno dell'appendice \hyperref[sec:OrderEdgesComplete]{Metodo OrderEdgesComplete} serve a tale scopo.

Questo metodo sfrutta algoritmi di ordinamento estremamente ottimizzati forniti direttamente dall'almbiente di sviluppo, considerando inoltre che la sua invocazione è necessaria una unica volta, può, e viene, utilizzato anche per trovare gli $m$ lati sopra citati sono inficiare in modo sensibile le prestazioni di \textbf{Build Model}.

\textbf{OrderEdgesComplete} restituisce una \textbf{lista di vettori di interi}, \textbf{listArray}, in cui l'$i-esimo$ elemento contiene, rispetto al nodo con indice di memorizzazione $i$, una sequenza dei restanti $n-1$ vertici basata sulla loro distanza crescente. In altre parole i lati il cui \textbf{upper bound} deve essere posto pari ad $1$ sono i soli individuabili dagli estremi

\begin{center}
$[0,(listArray[0])[0]], [0,(listArray[0])[1]], ..., [0,(listArray[0])[m]]$\\
...\\
$[i,(listArray[i])[0]], ..., [i,(listArray[i])[m]]$\\
...\\
$[n-1,(listArray[n-1])[0]], ..., [n-1,(listArray[n-1])[m]]$
\end{center}
...
La porzione di codice precedente viene inserita tra la fase di creazione delle variabili del modello matematico (vettore \textbf{x}) e la definizione dei vincoli indicate nel paragrafo \hyperref[sec:ModelloCSS]{Modello Matematico: Linguaggio C\#}.

In realtà anche la creazione delle variabili deve tenere conto di \textbf{nEdges} nel seguente modo:
...
Combinando quindi quanto detto nel paragrafo \hyperref[sec:ModelloCSS]{Modello Matematico: Linguaggio C\#} e le informazioni qui fornite, il metodo \textbf{BuildModel} completo risulta essere:
...
Terminata la fase euristica, prima di riapplicare il metodo \textbf{LOOP} sul modello matematico non privato dei vincoli di \textit{subtour elimination} trovati, è necessario riabilitare tutte le variabili. Questa operazione è eseguita dal metodo \textbf{ResetVariables}:
...
La classe \textbf{Stopwatch}, appartenente alla libreria \textbf{.NET Framework}, mette a disposizione del programmatore metodi e variabili che consentono di misurare il tempo trascorso in uno o più intervalli di tempo. Ai fini della tesi, sono stati utilizzati i metodi non statici \textbf{Start()} e \emph{Stop()} e la proprietà \textbf{ElapsedMilliseconds} di tipo long.

I metodi sopra citati, come facilmente intuibile, definiscono rispettivamente l'\textbf{istante} in cui la misurazione viene avviata e quello in cui viene arrestata. La proprietà determina invece la quantità di tempo trascorsa, espressa in \textbf{millisecondi}, dall' invocazione del metodo Start().

\'E fondamentale far notale che se all'interno di un certo thread \textbf{A} si esegue \textbf{Start()}, \textbf{nessuna} operazione successiva, che tolga o meno il focus ad \textbf{A} (avvio di altri threads, invocazione di metodi che interagiscono con librerie esterne..), compromette la misurazione del cronometro. Questo infatti si basa sull'orologio di sistema della macchina, chiaramente non influenzabile dall'applicazione.
...
La funzione \textbf{xPos} è realizzata per ottenere l'\textbf{indice} di memorizzazione di un generico \textbf{lato} all'interno di vettori e liste contenenti le \textbf{variabili} che li rappresentano. All'interno di \textit{xPos} è definita una apposita funzione matematica \textbf{F} che, dati gli indici di memorizzazione \textbf{i} e \textbf{j} ($0\leq i<j<n$) dei due nodi che delimitano il lato, restituisce un valore compreso tra $0$ e $((n - 1) * n / 2) - 1$, dove $n$ è il numero di nodi che compongono il grafo.

Si pensi ad una matrice $M_{(n, n)}$ in cui l'elemento $a_{i,j}$ corrisponde alla variabile associata al lato delimitato dai nodi con indice di memorizzazione \textbf{i} e \textbf{j}. Solamente una porzione di $M$ è \textit{necessaria} in quanto $a_{i,j}$ è equivalente a $a_{j,i}$ ed $a_{i,i}$ rappresenta un \textit{cappio}. Nel complesso quindi degli $n^2$ elementi solamente $(n^2 - n)/2 \rightarrow (n - 1) * n / 2$ sono \textit{validi}.

Immaginiamo di inserire questi ultimi in un vettore \textbf{v} ordinandoli secondo una visita \textit{row wise} della matrice, la funzione \textbf{F} deve restituire l'indice di memorizzazione in \textbf{v} secondo tale logica.

Per quanto detto, seguendo una visita \textit{row wise} di \textbf{M}, giunti ad un qualsiasi elemento valido della \textbf{i-esima} riga, sono stati incontrati:

\begin{eqnarray}
\displaystyle\sum\limits_{k=0}^{i+1} k = \displaystyle\sum\limits_{k=0}^{i} k + (i+1)  = \displaystyle\frac{i(i+1)}{2} + (i+1) = \displaystyle\frac{(i+1)(i+2)}{2}
\end{eqnarray}

elementi \textbf{non validi}. Nel complesso \textbf{F} deve quindi essere:

\begin{eqnarray}
&\displaystyle F(i,j)=
\begin{cases}
\text{impossibile} & \text{se i = j}\\
F(j,i) & \text{se i > j} \\
i * n + j - (i + 1) * (i + 2) / 2 & \text{altrimenti}
\end{cases}
\end{eqnarray}

Dove non sono considerati i casi di indici \textit{out of bound}\footnote{negativi o superiore ad $n-1$}.

La versione algoritmica di \textbf{xPos} diventa quindi:
...
\begin{itemize}
    \item \textbf{i}: primo indice di memorizzazione del primo nodo su cui incide il lato;
    \item \textbf{j}: secondo indice di memorizzazione del primo nodo su cui incide il lato;
    \item \textbf{nNodes}: numero totale di nodi e quindi il numero di righe e colonne della matrice \textbf{M} descritta;
\end{itemize}

\begin{lstlisting}

//-1 viene interpretato come errore
if (i == j)
    return -1;
...
All'interno di questa sezione viene mostrato l'adattamento dell'algoritmo di \textbf{Kruscal} che in un grafo connesso individua l'albero a costo minimo, per determinare i \textit{subtour} all'interno di una potenziale soluzione del \textit{TSP} studiato e generarne i relativi vincoli di eliminazione.

Prima di passare all'algoritmo vero e proprio se ne discute il funzionamento. Inizialmente si definiscono $n$ \textit{componenti connesse} (\textbf{cc}), ognuna delle quali contenente un distinto nodo del problema. Questa banale inizializzazione è realizzata dal metodo \textbf{InitCC}.

Successivamente, si opera iterativamente su ogni lato della soluzione nel seguente modo: le due \textit{cc} a cui appartengono i nodi che li delimitano vengono unificate. Nel caso in cui queste siano già uguali significa che un \textbf{subtour} è stata trovato e se ne può definire il vincolo di eliminazione. Il metodo \textbf{UpdateCC} esegue una sola operazione di aggiornamento e pertanto deve essere invocato per tutti gli $n$ lati che definiscono la soluzione presa in esame.


Durante l'intero algoritmo si fa riferimento al vettore di interi \textbf{cc} contenente all'indice $i-esimo$ l'identificativo della \textit{componente connessa} a cui appartiene il nodo con indice di memorizzazione $i$\footnote{Si ricorda che l'indice di memorizzazione di un qualsiasi nodo è inferiore di una unità rispetto al suo indice reale.}. L'inizializzazione di questo vettore viene fornita dal metodo \textbf{InitCC}:
...
In altre parole l'identificativo delle $n$ iniziali \textit{componenti connesse} corrisponde all'indice di memorizzazione del primo nodo assegnatovi.
...
\begin{itemize}
	\item \textbf{cplex}: oggetto \textit{contenente} il modello matematico corrente, eventualmente necessario per la creazione del vincolo di subtour elimination;
	\item \textbf{x}: vettore contenente le variabili del modello, eventualmente necessario per la creazione del vincolo di \textit{subtour elimination};
	\item \textbf{rcExpr}: lista di \textbf{ILinearNumExpr} all'interno della quale vengono memorizzate le espressioni dei vincoli di \textit{subtour elimination};
	\item \textbf{bufferCoeffRC}: lista di interi contenente i termini noti dei vincoli di \textit{subtour elimination};
	\item \textbf{relatedComponents}: vettore che definisce per ogni nodo del grafo l'identificativo della componente connessa di cui fa parte attualmente;
	\item \textbf{i}: indice di memorizzazione del primo nodo che delimita il lato preso in esame durante questa chiamata;
	\item \textbf{j}: indice di memorizzazione del secondo nodo che delimita il lato preso in esame durante questa chiamata;
\end{itemize}

Come preannunciato il metodo \textit{UpdateCC} esegue due operazioni distinte nel caso in cui il lato analizzato presenta o meno la medesima \textit{componente connessa} per i due nodi che lo delimitano. Il secondo caso viene gestito quindi nel seguente modo:
...
Ripetere il metodo \textbf{UpdateCC} una ed una sola volta per ogni lato appartenente alla soluzione corrente assicura che le due liste \textbf{rcExpr} e \textbf{bufferCoeffRC} contengano tutti i dati per formulare i vincoli di \textit{subtour elimination} ricercati.

\subsection*{METODO ORDEREDGESCOMPLETE}
\label{sec:OrderEdgesComplete}
